 ...
 * limitations under the License.
 * =============================================================================
 */ge.wasm.wasmPaths={wasm:"../../ort-wasm-simd-threaded.jsep.wasm"};ge.wasm.numThreads=4;self.onmessage=async e=>{const{type:t}=e.data;if(t==="runInference")try{const r=await Ea.create("../../superpoint_lightglue_pipeline.ort.onnx"),a=["../../otwarcie_fabryczna_testowy.jpg","../../fabryczna_otwarcie_topo.jpg"],n=await Promise.all(a.map(_=>vg(_))),i=256,s=256,u=n.map((_,b)=>$g(_,i,s,b)),d=new Float32Array([...u[0],...u[1]]),c={images:new Fe("float32",d,[2,1,s,i])},f=performance.now(),m=await r.run(c),g=performance.now();self.postMessage({type:"inferenceComplete",data:{inferenceTime:g-f,results:m}})}catch(r){self.postMessage({type:"error",data:{message:r.message}})}};function $g(e,t,r,a){const i=new OffscreenCanvas(t,r).getContext("2d");i.drawImage(e,0,0,t,r);const s=i.getImageData(0,0,t,r).data,u=new Float32Array(t*r);for(let d=0;d<t*r;d++){const l=s[d*4]/255,c=s[d*4+1]/255,f=s[d*4+2]/255;u[d]=.299*l+.587*c+.114*f}return u}async function vg(e){const r=await(await fetch(e)).blob();return await createImageBitmap(r)}
