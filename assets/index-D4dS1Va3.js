 ...
 * limitations under the License.
 * =============================================================================
 */const s2={__name:"HomeView",setup(e){Le.wasm.numThreads=4,Le.wasm.wasmPaths={wasm:"./ort-wasm-simd-threaded.jsep.wasm"};async function t(a){return new Promise((o,u)=>{const l=new Image;l.crossOrigin="anonymous",l.onload=()=>o(l),l.onerror=u,l.src=a})}function r(a,o,u,l){console.time(`Preprocess Image Time ${l}`);const c=document.createElement("canvas");c.width=o,c.height=u;const p=c.getContext("2d");p.drawImage(a,0,0,o,u);const h=p.getImageData(0,0,o,u).data,m=new Float32Array(o*u);for(let g=0;g<o*u;g++){const y=h[g*4]/255,x=h[g*4+1]/255,w=h[g*4+2]/255;m[g]=.299*y+.587*x+.114*w}return console.timeEnd(`Preprocess Image Time ${l}`),m}async function n(){return await Uo.create("./superpoint_lightglue_pipeline.ort.onnx")}const s=hh(null);async function i(){try{const a=await n(),o=["./otwarcie_fabryczna_testowy.jpg","./fabryczna_otwarcie_topo.jpg"],u=await Promise.all(o.map(b=>t(b))),l=256,c=256,p=u.map((b,k)=>r(b,l,c,k));await new Promise(b=>setTimeout(b,100));const h=new Float32Array([...p[0],...p[1]]),g={images:new Mt("float32",h,[2,1,c,l])},y=performance.now(),x=await a.run(g),w=performance.now();s.value=`${(w-y).toFixed(2)} ms`,console.log("Inference results:",x)}catch(a){console.error(`Failed to inference ONNX model: ${a}`)}}return i(),(a,o)=>(Gh(),Xw("main",null,[o[0]||(o[0]=Zh(" Hello World ")),Ao("p",null,"Inference Time: "+Qf(s.value),1)]))}},a2=Nv({history:pv("/"),routes:[{path:"/",name:"home",component:s2}]}),Qy=R$(Dv);console.log("a change4");Qy.use(a2);Qy.mount("#app");
