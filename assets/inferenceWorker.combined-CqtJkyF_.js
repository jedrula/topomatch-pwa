...
if (local_idx == 0) {
let sum_final = ${We("workgroup_shared[0][0]",u)} / f32(hight * ${u});
let squared_sum_final = ${We("workgroup_shared[0][1]",u)} / f32(hight * ${u});
...
let mean = ${We("mean_vector",f)} / uniforms.norm_size;
let inv_std_dev = inverseSqrt(${We("mean_square_vector",f)} / uniforms.norm_size ${r?"":"- mean * mean"} + uniforms.epsilon);
...
let mean = ${We("sum",_)} / f32(uniforms.hidden_size);
let inv_std_dev = inverseSqrt(${We("square_sum",_)} / f32(uniforms.hidden_size) ${a?"":"- mean * mean"} + uniforms.epsilon);
...
if (lindex == 0) {
rowSumShared = ${w}(${We("threadShared[0]",h)});
}
workgroupBarrier();
 ...
 * limitations under the License.
 * =============================================================================
 */typeof rp=="object"&&typeof Gr=="object"&&(Gr.exports=ei);console.log("wow3");ei.env.wasm.numThreads=4;self.onmessage=async be=>{const{type:ne,userImageBuffer:fe,topoImageBuffer:De}=be.data;if(ne==="createSession"){console.log("createSession in worker 2");try{const de=performance.now(),T=await ei.InferenceSession.create("../../superpoint_lightglue_pipeline.ort.onnx",{executionProviders:["wasm"],graphOptimizationLevel:"all",wasm:{numThreads:navigator.hardwareConcurrency?Math.max(1,Math.min(4,navigator.hardwareConcurrency)):2,simd:!0,threads:!0}}),ce=performance.now();self.session=T,self.postMessage({type:"sessionCreated",data:{sessionTime:ce-de}})}catch(de){self.postMessage({type:"error",data:{message:de.message}})}}if(ne==="runInference"){if(!self.session){self.postMessage({type:"error",data:{message:"Session is not initialized."}});return}let de=null,T=null,ce=null,Le=null;try{if(!fe||!De){self.postMessage({type:"error",data:{message:"Both user and topo images must be provided."}});return}ce=new Blob([fe]),de=await createImageBitmap(ce),Le=new Blob([De]),T=await createImageBitmap(Le);const pe=[de,T],Se=256,ge=256,ve=pe.map((lt,_e)=>wc(lt,Se,ge,_e)),zt=new Float32Array([...ve[0],...ve[1]]),Et={images:new ei.Tensor("float32",zt,[2,1,ge,Se])},ii=performance.now(),Ct=await self.session.run(Et),ri=performance.now();self.postMessage({type:"inferenceComplete",data:{inferenceTime:ri-ii,results:Ct,images:pe,imgWidth:Se,imgHeight:ge}})}catch(pe){self.postMessage({type:"error",data:{message:pe.message}})}finally{de&&typeof de.close=="function"&&de.close(),T&&typeof T.close=="function"&&T.close(),de=null,T=null,ce=null,Le=null}}};function wc(be,ne,fe,De){const T=new OffscreenCanvas(ne,fe).getContext("2d");T.drawImage(be,0,0,ne,fe);const ce=T.getImageData(0,0,ne,fe).data,Le=new Float32Array(ne*fe);for(let pe=0;pe<ne*fe;pe++){const Se=ce[pe*4]/255,ge=ce[pe*4+1]/255,ve=ce[pe*4+2]/255;Le[pe]=.299*Se+.587*ge+.114*ve}return Le}});export default bc();
