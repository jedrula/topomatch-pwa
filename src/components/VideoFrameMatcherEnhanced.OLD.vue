...
// Helper functions removed - analysis now happens in store
...
const processVideo = async () => {
  ...
  try {
    ...
    if (createdAscentId.value) {
      ...
      // Pass boulder problems to store for autonomous matching
      const problemsWithHolds = boulderProblemsStore.boulderProblems.filter(
        p => p.holds && p.holds.length > 0
      );
      
      console.log(`📊 Pushing frames + ${problemsWithHolds.length} problems to analysis queue`);
      await analysisQueue.setFrames(createdAscentId.value, processedFrames, problemsWithHolds);
    }
...
// Image matching functions removed - analysis now happens in store
