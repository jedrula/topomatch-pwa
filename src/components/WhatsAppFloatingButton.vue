<template>
  <!-- WhatsApp Floating Button - only for logged in users on web (not native) -->
  <a
    v-if="userStore.user && !isNativePlatform"
    :href="whatsappUrl"
    target="_blank"
    rel="noopener noreferrer"
    :class="[
      'fixed bottom-4 left-4 bg-green-500 hover:bg-green-600 text-white rounded-full shadow-lg flex items-center justify-center transition-all active:scale-95 z-40',
      showText ? 'w-auto h-12 px-4 gap-2' : 'w-12 h-12'
    ]"
    :title="showText ? '' : 'Help & Feedback'"
  >
    <!-- WhatsApp Icon -->
    ...
    <span v-if="showText" class="text-sm font-medium whitespace-nowrap">Help & Feedback</span>
  </a>
</template>
