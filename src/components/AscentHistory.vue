<template>
  <div class="space-y-4">
    <!-- Ascent Statistics -->
    <div v-if="ascentStore.ascentStats" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">Ascent Statistics</h3>
      
      <div class="grid grid-cols-2 gap-4 text-sm">
        <div class="text-center p-3 bg-gray-50 rounded-lg">
          <div class="text-2xl font-bold text-blue-600">{{ ascentStore.ascentStats.totalAscents }}</div>
          <div class="text-gray-600">Total Sends</div>
        </div>
        <div class="text-center p-3 bg-gray-50 rounded-lg">
          <div class="text-2xl font-bold text-green-600">{{ ascentStore.ascentStats.uniqueClimbers }}</div>
            ...
            <div class="flex items-center space-x-2">
              <div class="w-16 bg-gray-200 rounded-full h-2">
                <div 
                  ...
                  :style="{ width: `${(ascentStore.ascentStats.attemptTypes.flash / ascentStore.ascentStats.totalAscents) * 100}%` }"
                ></div>
              </div>
            ...
            <div class="flex items-center space-x-2">
              <div class="w-16 bg-gray-200 rounded-full h-2">
                <div 
                  ...
                  :style="{ width: `${(ascentStore.ascentStats.attemptTypes.second / ascentStore.ascentStats.totalAscents) * 100}%` }"
                ></div>
              </div>
            ...
            <div class="flex items-center space-x-2">
              <div class="w-16 bg-gray-200 rounded-full h-2">
                <div 
                  ...
                  :style="{ width: `${(ascentStore.ascentStats.attemptTypes.multiple / ascentStore.ascentStats.totalAscents) * 100}%` }"
                ></div>
              </div>
              <span class="w-8 text-right">{{ ascentStore.ascentStats.attemptTypes.multiple }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Average User Grade -->
      <div v-if="ascentStore.ascentStats.averageUserGrade" class="mt-4 pt-4 border-t border-gray-100">
        <div class="flex items-center justify-between text-sm">
          <span class="text-gray-600">Community Grade Opinion</span>
          <span class="font-medium text-gray-900">{{ ascentStore.ascentStats.averageUserGrade }}</span>
        </div>
        <p class="text-xs text-gray-500 mt-1">
          Based on {{ ascentStore.ascentStats.userGrades.length }} user rating{{ ascentStore.ascentStats.userGrades.length !== 1 ? 's' : '' }}
        </p>
      </div>
    </div>

    <!-- Recent Ascents -->
    <div v-if="ascentStore.ascents.length > 0" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div class="space-y-3 max-h-64 overflow-y-auto">
        ...
        >
          <div class="flex-1">
            ...
            <div v-if="ascent.notes" class="text-sm text-gray-700 mt-1">
              "{{ ascent.notes }}"
            </div>
          </div>
      ...
      <div v-if="ascentStore.ascents.length > 10" class="mt-3 text-center">
        <p class="text-sm text-gray-500">
          Showing 10 of {{ ascentStore.ascents.length }} ascents
        </p>
      </div>
    </div>

    <!-- No Ascents Yet -->
    <div v-else-if="!ascentStore.isLoading" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
    ...
    <!-- Loading State -->
    <div v-if="ascentStore.isLoading" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
