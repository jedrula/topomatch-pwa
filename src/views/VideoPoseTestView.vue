    ...
    <div class="max-w-4xl mx-auto px-4">
      <div class="text-center mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">
          Video Pose Analysis Test
        </h1>
        <p class="text-gray-600">
          Upload a climbing video to extract 3 frames, detect poses, and project them onto matched images
        </p>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <!-- No images warning -->
        <div v-else-if="comparisonImages.length === 0" class="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div class="flex items-start space-x-3">
            <svg class="w-5 h-5 text-yellow-600 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L2.732 15.5c-.77.833.192 2.5 1.732 2.5z"></path>
            </svg>
            <div>
              <h4 class="text-sm font-medium text-yellow-800">No Comparison Images</h4>
              <p class="text-sm text-yellow-700 mt-1">
                No images with boulder problems found. Please create some locations with images and boulder problems first.
              </p>
      ...
      <div v-if="results.frames.length > 0" class="mt-8 space-y-6">
            ...
            <div v-for="(frame, index) in results.frames" :key="index" class="space-y-2">
              ...
              <div v-if="frame.poseData" class="text-xs text-gray-600 bg-gray-50 p-2 rounded">
                ...
                <div class="grid grid-cols-2 gap-1">
                  <div>L.Wrist: {{ frame.poseData.keypoints.leftWrist ? `(${Math.round(frame.poseData.keypoints.leftWrist.x)}, ${Math.round(frame.poseData.keypoints.leftWrist.y)}) ${(frame.poseData.keypoints.leftWrist.confidence * 100).toFixed(1)}%` : 'Not detected' }}</div>
                  <div>R.Wrist: {{ frame.poseData.keypoints.rightWrist ? `(${Math.round(frame.poseData.keypoints.rightWrist.x)}, ${Math.round(frame.poseData.keypoints.rightWrist.y)}) ${(frame.poseData.keypoints.rightWrist.confidence * 100).toFixed(1)}%` : 'Not detected' }}</div>
                  <div>L.Ankle: {{ frame.poseData.keypoints.leftAnkle ? `(${Math.round(frame.poseData.keypoints.leftAnkle.x)}, ${Math.round(frame.poseData.keypoints.leftAnkle.y)}) ${(frame.poseData.keypoints.leftAnkle.confidence * 100).toFixed(1)}%` : 'Not detected' }}</div>
                  <div>R.Ankle: {{ frame.poseData.keypoints.rightAnkle ? `(${Math.round(frame.poseData.keypoints.rightAnkle.x)}, ${Math.round(frame.poseData.keypoints.rightAnkle.y)}) ${(frame.poseData.keypoints.rightAnkle.confidence * 100).toFixed(1)}%` : 'Not detected' }}</div>
                </div>
                <div class="mt-1">Overall Confidence: {{ (frame.poseData.confidence * 100).toFixed(1) }}%</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="results.match" class="bg-white rounded-lg shadow p-6">
          <h3 class="text-lg font-medium text-gray-900 mb-4">Best Matching Image with Projected Poses</h3>
          ...
          <div class="flex items-start space-x-6">
            <div class="flex-1">
              ...
              <div class="mt-2 text-sm text-gray-600">
                ...
                <div class="mt-2 text-xs text-blue-600">Poses projected using homography transformation</div>
              </div>
            </div>
          </div>
        </div>
...
const loadComparisonImages = async () => {
  ...
  try {
    ...
    for (const location of locations) {
      try {
        ...
        for (const image of locationImages) {
          try {
            const boulderProblems = await boulderProblemsService.getBoulderProblemsByImage(location.id, image.id);
    ...
    if (allImages.length === 0) {
      console.warn('No images with boulder problems found. Make sure to add some boulder problems first.');
    }
  } catch (err) {
...
const drawProjectedPoses = () => {
  ...
  results.value.frames.forEach((frame, frameIndex) => {
    ...
    console.log(`🎯 Processing frame ${frameIndex + 1} with ${frame.pose.keypoints.length} keypoints`);

    frame.pose.keypoints.forEach((keypoint, keypointIndex) => {
      if (keypoint.score < 0.3) return; // Skip low-confidence keypoints

      const keypointName = frame.pose.keypointNames?.[keypointIndex] || `kp${keypointIndex}`;
      
      console.log(`🔸 Processing ${keypointName}: (${keypoint.x}, ${keypoint.y}) conf: ${keypoint.score.toFixed(2)}`);
      ...
      console.log(`✨ Transformed ${keypointName}:`, 
                  `(${keypoint.x}, ${keypoint.y}) → ${transformedPoint ? `(${transformedPoint.x.toFixed(1)}, ${transformedPoint.y.toFixed(1)})` : 'null'}`);

      if (transformedPoint) {
        // Scale the transformed point to display size
        const x = transformedPoint.x * scaleX;
        const y = transformedPoint.y * scaleY;
...
const drawPoseOnFrame = (frameIndex) => {
  ...
  const scaleX = rect.width / frame.width;  // frame.width is the original video width
  const scaleY = rect.height / frame.height; // frame.height is the original video height
  ...
  if (keypoints.leftWrist && keypoints.rightWrist && 
      keypoints.leftWrist.confidence > 0.3 && keypoints.rightWrist.confidence > 0.3) {
    ctx.beginPath();
  ...
  if (keypoints.leftAnkle && keypoints.rightAnkle && 
      keypoints.leftAnkle.confidence > 0.3 && keypoints.rightAnkle.confidence > 0.3) {
    ctx.beginPath();
  ...
  if (results.value.frames?.length) {
    results.value.frames.forEach((frame, idx) => {
      console.log(`Frame ${idx + 1}:`, {
        hasPose: !!frame.pose,
        keypointCount: frame.pose?.keypoints?.length || 0,
        sampleKeypoints: frame.pose?.keypoints?.slice(0, 3).map(kp => ({
          x: kp.x,
          y: kp.y,
          score: kp.score
        }))
